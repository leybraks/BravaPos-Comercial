  ...
  return (
    <>
      ...
      <div className={contenedorClasses} style={{ maxHeight: '100%' }}>
        ...
        <div className={`px-6 pb-4 flex justify-between items-start border-b shrink-0 ${esDesktop ? 'pt-6' : 'pt-1'} ${tema === 'dark' ? 'border-[#222]' : 'border-gray-100'}`}>
          <div>
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            {!esDesktop && (
              <button
                onClick={() => setCarritoAbierto(false)}
                className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-xl transition-colors active:scale-95 ${tema === 'dark' ? 'bg-[#222] text-neutral-400 hover:text-white' : 'bg-gray-100 text-gray-500 hover:text-gray-900'}`}
              >
                ✕
              </button>
            )}
                  ...
                  <div key={item.cart_id || item.id} className={`p-4 sm:p-5 rounded-3xl border shadow-sm flex flex-col gap-4 ${tema === 'dark' ? 'bg-[#1a1a1a] border-[#333]' : 'bg-white border-gray-200'}`}>
                    ...
                    <div className={`pt-4 border-t flex justify-between items-center gap-2 ${tema === 'dark' ? 'border-[#333]' : 'border-gray-100'}`}>
                      <button
                        ...
                        className={`px-5 py-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-colors active:scale-95 flex items-center gap-2 ${tema === 'dark' ? 'bg-[#2a2a2a] text-neutral-300 hover:bg-[#333]' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                      >
                        📝 Notas
                      </button>
        ...
        <div className={`p-4 sm:p-6 border-t flex flex-col gap-3 shrink-0 ${tema === 'dark' ? 'border-[#222] bg-[#0d0d0d]' : 'border-gray-200 bg-white'}`}>
          {carrito.length > 0 ? (
            <button
              ...
              disabled={procesando}
              className="w-full text-white rounded-2xl h-16 sm:h-20 font-black text-xl sm:text-2xl flex justify-center items-center transition-all active:scale-[0.98]"
              style={{ backgroundColor: colorPrimario, boxShadow: `0 8px 25px ${colorPrimario}50` }}
            >
