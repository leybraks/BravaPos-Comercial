...
export default function MesasLlevar({
          ...
          return (
            <div key={orden.id} className={`p-5 rounded-3xl flex justify-between items-center relative overflow-hidden transition-all ${tema === 'dark' ? 'bg-[#1a1a1a] border border-[#2a2a2a] hover:bg-[#222]' : 'bg-white border border-gray-200 shadow-sm hover:shadow-md'}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                {!estaPagado ? (
                  <button onClick={() => setOrdenACobrar(orden)} style={{ backgroundColor: colorPrimario }} className="text-white px-4 py-2 rounded-2xl font-black text-xs md:text-sm shadow-md active:scale-95 transition-all">COBRAR</button>
                ) : (
                  estaListo && (
