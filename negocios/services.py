...
def _evaluar_condiciones_regla(regla, orden, metodo_lower, ahora):
    ...
    # Día de la semana (si está configurado)
...
def _calcular_descuento_happy_hours(orden, ahora):
    ...
    Evalúa cada detalle contra los HorarioVisibilidad activos.
    Retorna (descuento_hh, lineas_hh) donde lineas_hh es una lista
    de dicts para mostrar en el frontend.
    """
    ...
    # Traemos todos los happy hours activos del negocio
    ...
    # Filtramos los que aplican hoy en día/hora/fecha
    hhs_activas = []
    for hh in hhs:
        ...
        # Verificar rango de fechas (si no se repite semanalmente)
    ...
    for hh in hhs_activas:
        if hh.tipo_promo == 'visibilidad':
            continue  # Solo visibilidad, no afecta precio

        for detalle in orden.detalles.all():
            producto = detalle.producto

            # Verificar si el HH aplica a este producto o categoría
            aplica = False
            ...
            if hh.tipo_promo == 'precio_especial' and hh.precio_especial:
                ...
                # Ej: compra_x=2, lleva_y=1 → por cada 2, 1 es gratis
...
def aplicar_reglas_negocio(orden, metodo_pago=None):
    ...
    - metodo_pago: método de pago conocido al momento del cobro ('yape', 'efectivo', etc.).
      Si es None se lee orden.metodo_pago_esperado.
    ...
    for regla in reglas:
        ...
        # Tipos predefinidos (compatibilidad hacia atrás)
        if regla.tipo == 'recargo_llevar' and orden.tipo == 'llevar':
            ...
            # Regla personalizada — accion_es_descuento define si es descuento o recargo
